...
<body>
    ...
    <div class="container-fluid mt-4">
        <div class="row ">
           ...
           <div class="col-9">
               <h6 class="lead my-3 ms-5">Trash Mail</h6>
               <table class="table">
                   <?php
                   $loginedUserId = $user['id'];
                   $callingInbox = mysqli_query($connect,"select * from mails JOIN accounts ON mails.Sender_id=accounts.id where (reciever_id='$loginedUserId' OR sender_id='$loginedUserId') AND status='-1' ORDER BY mail_id DESC");
    ...
    <?php include "include/footer.php";?>
      ...
      if(isset($_POST['send'])){
        ...
        $status = 1; //1 = mail send(inbox) // 2= draft (saved mail) // 3. (trashed)
